# You Don't Know Rämpf — Sound Concept
...
| Tool | Purpose | Output |
|------|---------|--------|
| **Suno v5** | Music & jingles — anything with melody, rhythm or vocals | `.mp3` stems |
| **ElevenLabs SFX** | Short sound effects — clicks, whooshes, ticks | `.mp3` one-shots |

### Tips for Suno v5

- **Spoken Word Problem**: Suno v5 singt manchmal `[Spoken]`-Parts. Workaround:
  Nutze `[Rap]` oder `[Talk-Sing]` statt `[Spoken]` — das hält den Rhythmus und
...
- **Loop-Musik**: Für die Timer-Musik "loop-friendly" im Style angeben und
...
- **Mehrere Varianten**: Wie bei Suno, 3–4 Varianten generieren und die
  passendste wählen.
...
> Kurzer Energiestoß wenn eine neue Runde startet. Knackig, keine Langeweile.
...
### 3. Beleidigungsfechten — Runden-Intro

> Spezial-Jingle für die Monkey Island Insult-Sword-Fighting-Runde.
...
> Hintergrundmusik während die Spieler nachdenken. Soll Spannung aufbauen ohne
...
### 5. Richtige Antwort — Sting

> 3–5 Sekunden Jubel. Kurz, knackig, befriedigend.
...
> Episches Ende. Der Gewinner steht fest. Übertrieben dramatisch, aber mit
> Augenzwinkern.
...
> the SFX column shows "—" with a reference.

| Phase | Trigger | SFX | ElevenLabs Prompt | Reasoning |
|-------|---------|-----|-------------------|-----------|
| title→roundIntro | Mod clicks "Spiel starten" | Game start whoosh | `dramatic cinematic whoosh transition, deep bass sweep left to right, 1 second` | Marks transition from idle to game |
...
| question | Timer hits 0 | Time-up buzzer | `harsh game show buzzer, time is up, short and punchy, 0.8 seconds` | Hard stop — time's over |
| question→reveal | Mod clicks "Antwort aufdecken" | — | — | Covered by Suno sting #7 (Antwort-Reveal) |
| reveal | Correct highlight shown | — | — | Covered by Suno sting #5 (Richtige Antwort) |
| reveal | Wrong highlight shown | — | — | Covered by Suno sting #6 (Falsche Antwort) |
| reveal | Roast text appears | — | — | Covered by Suno sting #8 (Roast) |
| reveal→question | Mod clicks "Weiter" | Next question transition | `quick light whoosh, page turn transition, subtle and fast, 0.4 seconds` | Quick swoosh moving on to the next question |
...
| scoreboard→roundIntro | Mod clicks "Nächste Runde" | — | — | Covered by round transition swoosh |
| finale | Final scores shown | — | — | Covered by Suno finale #9 |
...
|------|-------|------|-------|--------|
| 1 | Intro / Titelmusik | Suno | 60–90s | Sets the tone, needed for title screen |
